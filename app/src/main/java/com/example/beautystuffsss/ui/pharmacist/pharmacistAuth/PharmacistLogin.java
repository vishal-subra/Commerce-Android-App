...
public class PharmacistLogin extends AppCompatActivity {
    ...
    String TAG = "Pharmacist_login";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_pharmacist_login);
        emailEditText = findViewById(R.id.pharmacist_login_editText_email);
        passwordEditText = findViewById(R.id.pharmacist_login_editText_password);
        registerTextView = findViewById(R.id.pharmacist_login_textView_register_here);
        loginButton = findViewById(R.id.pharmacist_login__login_button);
        progressDialog = new ProgressDialog(this);
        preferences = new Preferences(this);
        mAuth = FirebaseAuth.getInstance();
        firestore = FirebaseFirestore.getInstance();
        registerTextView.setOnClickListener(v -> startActivity(new Intent(PharmacistLogin.this, PharmacistRegister.class)));
        loginButton.setOnClickListener(v -> {
            ...
            if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                ...
                passwordEditText.setError("Can't login without password");
                    ...
                    if (task.isSuccessful()) {
                        ...
                        firestore.collection(Constants.users).document(uId).get().addOnCompleteListener(task1 -> {
                            if (task1.isSuccessful()) {
                                ...
                                if (result.getString(Constants.currentUserType).equals(Constants.uTypePharmacist)) {
                                    try {
                                    ...
                                    emailEditText.setError("Email not registered as pharmacist");
                                }
                            } else {
                                progressDialog.dismiss();
